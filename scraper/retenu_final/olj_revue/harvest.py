"""
Orchestration de la recolte : N articles par entree du registre MEDIA_REVUE_REGISTRY.json.

Flux par media :
  1. Resoudre la liste des URLs de hub (opinion_hub_urls, ou RSS, ou url) + overrides
     locaux (HUB_DISCOVERY_OVERRIDES) sans toucher au JSON backend.
  2. Pour chaque hub : discover_article_links (Playwright + filtre smart_content).
  3. Pour chaque lien candidat : fetch_article (cascade scrape_cascade) jusqu'a N succes.
  4. Si pas assez d'articles : repli scrape(hub) (agregat Haaretz/IH ou page hub complete).

Sortie : JSON par article, meta.json et summary.json (voir run_harvest.py).
"""
