...
Documentation longue / brouillons : dossier local `archive/` (ignoré par git) — y compris les **runbooks & déploiement** retirés du dépôt (`archive/docs-ops-2026-04-06/`). **Carte `docs/`** : `docs/README.md`. **Onboarding agents** : `docs/ONBOARDING.md`.
...
- **Pipeline dual 9h/16h Paris** : le pipeline complet (collecte → traduction → pertinence → embeddings → clustering → sujets → génération draft) tourne à **9h Paris** (lun–ven). Un **refresh léger** tourne à **16h Paris** (mar–ven) : re-collecte + traduction + pertinence des nouveaux articles + soft-assign aux clusters existants (pas de re-HDBSCAN). Toggle front « Avec/Sans actualisation 16h » sur la page Édition pour filtrer les articles arrivés après 13h UTC.
