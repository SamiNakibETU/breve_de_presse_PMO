...
**2.6 — Score de pertinence éditoriale LLM (PROMU DE P2 À P0)**

C'est le fix le plus impactant. Les mots-clés lifestyle ne suffiront jamais — le run du 22 mars montre que des articles sur le basket turc, le cinéma Kafka, l'herméneutique islamique et les hommages funéraires passent le filtre. Seul un LLM peut discriminer « politique régionale du Moyen-Orient » de « culture/sport/religion non-politique ».

Fichier : `backend/src/services/relevance_scorer.py`

Prompt : `prompt_relevance_score_v2` (cf. MEMW_PROMPT_SUITE_v2.md, Section PROMPT 5).
Modèle : Haiku 4.5 · Température : 0.0 · Coût : ~$0.0005/article (~$0.15/run pour 300 articles).

```python
async def score_relevance(title_fr: str, summary_fr: str, media_name: str, country_code: str) -> dict:
    """
    Retourne: {
        "relevance_score": 0.0-1.0,
        "relevance_band": "core" | "related" | "tangential" | "out_of_scope",
        "reasoning": "..."
    }
    """
```

**Intégration dans le pipeline** : exécuté **après la traduction, avant la dédup**. Les articles avec `relevance_band == "out_of_scope"` (score < 0.40) sont exclus du clustering. Les `"tangential"` (0.40-0.69) sont inclus mais marqués comme tels.

**Exemples de classification attendue** (basés sur le run du 22 mars) :

| Article | Score attendu | Band |
|---------|---------------|------|
| « Les attaques iraniennes exigent une rupture arabe » | 0.95 | core |
| « L'or s'effondre malgré les tensions géopolitiques » | 0.72 | related |
| « Le basket turc est compromis par des décisions fédérales » | 0.15 | out_of_scope |
| « Le film Franz, réalisé par Agnieszka Holland » | 0.08 | out_of_scope |
| « La sagesse est un chemin d'intégrité intérieure » | 0.12 | out_of_scope |
| « Fahrettin Paşa sauve le trésor de Médine » | 0.35 | out_of_scope |
| « Le Ramadan s'en va, mais l'espoir demeure » | 0.25 | out_of_scope |

**Impact attendu** : sur les 394 traduits du run actuel, estimation ~150-200 sont hors périmètre. Après filtrage, le corpus candidat passe de 394 à ~150-200, ce qui résout aussi le problème du méga-cluster (moins d'articles = clusters plus propres).

Champ ajouté à `articles` :
```python
relevance_score = Column(Float, nullable=True)
relevance_band = Column(String(20), nullable=True)  # core, related, tangential, out_of_scope
```

**2.7 — Diagnostic du méga-cluster (préparation Sprint 4)**

Le run du 22 mars produit 1 méga-cluster de 218 articles et 1 068 non classés. Causes identifiées :
1. HDBSCAN sur 1024 dimensions sans UMAP → malédiction de la dimensionnalité
2. Clustering sur tout le corpus (706 articles) au lieu des seuls candidats de l'Édition
3. Pas de filtre de pertinence → bruit massif dans le corpus
4. Paramètres HDBSCAN probablement `min_cluster_size` trop haut

Action Sprint 2 : documenter les paramètres HDBSCAN actuels (`min_cluster_size`, `min_samples`, `metric`) pour calibrer le Sprint 4.
...
- [ ] Le score de pertinence exclut les articles sport/culture/religion du clustering
- [ ] Sur 50 articles scorés manuellement : < 10% faux positifs (article géopol exclu à tort)
...
Paramètres HDBSCAN (corpus attendu 60-120 articles **après filtre pertinence + dédup**) :
- `min_cluster_size = 3` (vs. probablement 5-10 actuellement → cause du méga-cluster)
- `min_samples = 2` (vs. probablement 5+ actuellement → cause des 1068 non classés)
- `cluster_selection_method = 'leaf'` (produit des clusters fins, fusion par le Curateur)
- `metric = 'euclidean'` (sur les coordonnées UMAP, PAS cosine sur les embeddings bruts)

**IMPORTANT — Diagnostic du run 22 mars** :
- 218 articles dans 1 cluster + 1068 non classés = HDBSCAN échoue sur 1024 dimensions
- Le fix est UMAP (réduction à 15d) + filtre pertinence (÷2 le corpus) + dédup (÷2 encore)
- Résultat attendu : corpus de 60-120 articles → 8-15 clusters de 3-10 articles chacun
- Si > 20 clusters → `min_cluster_size` trop bas → augmenter à 4
- Si < 5 clusters → `min_cluster_size` trop haut → baisser à 2
...
06:00 UTC → Pipeline complet :
  ...
  2b. SCORE DE PERTINENCE ÉDITORIALE (Haiku, $0.0005/article)
     → relevance_score, relevance_band
     → out_of_scope (< 0.40) exclus de la suite
     → FILTRE CRITIQUE : élimine sport, culture, religion, nécrologies
